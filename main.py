...
# Пользователи, запускавшие бота, и записи о выдачах — это база клиентов, она попадает в /backup
USERS = dict(_stored.get("users", {}))       # user_id -> {username, first_seen}
GRANTS = dict(_stored.get("grants", {}))     # ключ -> {user_id, username, by, created}

# Админы, добавленные командой /addadmin (сверх базовых из BASE_ADMIN_IDS)
EXTRA_ADMINS = dict(_stored.get("extra_admins", {}))  # str(user_id) -> {username, added_by, added_at}
ADMIN_IDS = list(BASE_ADMIN_IDS)
for _uid in EXTRA_ADMINS:
    if _uid.lstrip("-").isdigit() and int(_uid) not in ADMIN_IDS:
        ADMIN_IDS.append(int(_uid))
...
# --- ВЫДАЧА ССЫЛОК В ПРИВАТЫ (вручную, через директ) ---
# Покупка внутри бота отключена: продажа идёт через директ (@manager), доступ выдаёт админ командой /grant.


def remember_user(user):
    """Запоминает ID и username пользователя: это база клиентов для /backup."""
    key = str(user.id)
    known = USERS.get(key)
    if known and known.get("username") == user.username:
        return
    USERS[key] = {
        "username": user.username,
        "first_seen": known["first_seen"] if known else datetime.now(timezone.utc).isoformat(timespec="seconds"),
    }
    save_state()


def who_is(user_id, username=None):
    return f"{user_id} (@{username})" if username else str(user_id)


async def deliver_private_links(user_id):
    """Создаёт одноразовые ссылки во все приваты и отправляет их пользователю.
    Возвращает (отправлено ли, [(номер, chat_id, ошибка) по неудачным], ошибка отправки)."""
    channels = list(PRIVATE_CHANNELS)
    if not channels:
        return False, [], "приваты не привязаны"

    links, failed = [], []
    for number, chat_id in enumerate(channels, start=1):
        try:
            # member_limit=1 → ссылка одноразовая; name помогает найти, кому она выдана
            invite = await bot.create_chat_invite_link(chat_id=chat_id, member_limit=1, name=f"buy_{user_id}"[:32])
            links.append((number, chat_id, invite.invite_link))
        except Exception as e:
            failed.append((number, chat_id, e))

    text = ""
    if links:
        text += "🎉 Твои одноразовые ссылки для входа в приваты:\n\n"
        text += "\n".join(f"Приват {n}: {url}" for n, _, url in links)
    ...
    try:
        await bot.send_message(chat_id=user_id, text=text.strip())
    except Exception as e:
        # Пользователю не написать (например, он заблокировал бота): созданные ссылки отзываем
        for _, chat_id, url in links:
            try:
                await bot.revoke_chat_invite_link(chat_id=chat_id, invite_link=url)
            except Exception:
                pass
...
async def grant_cmd(message: types.Message, command: CommandObject):
    """Выдача одноразовых ссылок покупателю, который оплатил в директе."""
    if not message.from_user or message.from_user.id not in ADMIN_IDS: return
    arg = (command.args or "").strip()
    if not arg.isdigit():
        await message.answer("Напиши ID пользователя: /grant 123456789")
        return
    if not PRIVATE_CHANNELS:
        await message.answer("Приваты не привязаны: сначала /private_links или /private_here.")
        return

    target = int(arg)
    sent, failed, error = await deliver_private_links(target)
    if not sent:
        await message.answer(
            f"❌ Не удалось отправить ссылки пользователю {target}: {error}\n"
            "Скорее всего, он ещё не запускал бота (пусть напишет боту /start) или заблокировал его."
        )
        return

    who = who_is(target, USERS.get(str(target), {}).get("username"))
    GRANTS[f"{int(time.time())}-{target}"] = {
        "user_id": target, "username": USERS.get(str(target), {}).get("username"),
        "by": message.from_user.id, "created": time.time(),
    }
    save_state()
    text = f"✅ Ссылки отправлены пользователю {who}."
    if failed:
        text += "\n⚠️ Не все приваты удалось выдать: " + "; ".join(f"приват {n} ({cid}): {e}" for n, cid, e in failed)
    await message.answer(text)


@dp.message(Command("backup"))
async def backup_cmd(message: types.Message):
    """Присылает админу файл данных: клиенты, выдачи, приваты, пул автопостов (на случай бана канала)."""
    if not message.from_user or message.from_user.id not in ADMIN_IDS: return
    if message.chat.type != "private":
        await message.answer("Резервную копию можно запросить только в личке с ботом.")
        return
    save_state()
    if not os.path.exists(DATA_FILE):
        await message.answer("Файла данных пока нет.")
        return
    stamp = datetime.now(timezone.utc)
    await message.answer_document(
        document=types.FSInputFile(DATA_FILE, filename=f"bot_data_{stamp:%Y%m%d_%H%M}.json"),
        caption=(
            f"Резервная копия. Пользователей: {len(USERS)}, выдач приватов: {len(GRANTS)}, "
            f"видео в пуле автопостов: {len(AUTOPOST['pool'])}.\n"
            "Чтобы восстановить, положи файл под именем bot_data.json в папку данных бота."
        ),
    )
...
async def start_post(message: types.Message, state: FSMContext):
    ...
    if not get_channel_id():
        ...
        return
    await message.answer("Отправь видео для поста:")
...
async def main():
    ...
    try:
        # Сообщение о запуске: сразу видно, сохранились ли данные и работает ли оплата
        ...
        await notify_admins(
            ...
            f"Приваты: {len(PRIVATE_CHANNELS)} из {PRIVATE_LIMIT} (выдаются командой /grant)\n"
